#!/usr/bin/env python3
"""Merge context-handoff hook entries into an existing .cursor/hooks.json."""
...
def hook_key(entry: dict) -> str:
    ...
    return f"{command}|{matcher}"


def merge_hooks(target: dict, fragment: dict) -> tuple[dict, int]:
    merged = target
    merged.setdefault("version", fragment.get("version", 1))
...
def main() -> int:
    if len(sys.argv) != 3:
        print("usage: merge_hooks.py <target-hooks.json> <fragment.json>", file=sys.stderr)
        return 2
