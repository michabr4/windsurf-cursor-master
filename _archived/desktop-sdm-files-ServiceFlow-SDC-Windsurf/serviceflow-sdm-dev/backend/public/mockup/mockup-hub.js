...
(function () {
  ...
  const WAVE_GRID_TITLE = {
    ...
    "support-api": "Cisco Support APIs",
  ...
  function getOverviewSyncStaticSuffix() {
    return (
  ...
  function renderOverviewSyncCards() {
    ...
    el.innerHTML = enabledHtml + getOverviewSyncStaticSuffix();
  }

  function parseWaveFromSourceNotes(notes) {
    if (!notes) return null;
    const m = /Wave\s*(\d+)/i.exec(String(notes));
    return m ? "W" + m[1] : null;
  }

  /** Live: integration sync strip from <code>/api/v1/admin/sources</code> (wave parsed from <code>notes</code>). */
  function renderOverviewSyncCardsFromRegistry(body) {
    const el = document.getElementById("overview-sync-cards");
    if (!el || !body || !Array.isArray(body)) return;
    const models = [];
    for (let i = 0; i < body.length; i++) {
      const r = body[i];
      if (!r || !r.enabled) continue;
      const w = parseWaveFromSourceNotes(r.notes || "");
      if (!w || !/^W\d+$/.test(w)) continue;
      models.push({ wave: w, waveNum: parseInt(w.slice(1), 10), row: r });
    }
    models.sort((a, b) => a.waveNum - b.waveNum);
    const enabledHtml = models
      .map((m) => {
        const r = m.row;
        const title = WAVE_GRID_TITLE[r.sourceName] || r.sourceName;
        const sched = r.schedule ? escHtml(String(r.schedule)) : "—";
        const upd = r.updatedAt ? escHtml(formatIsoShort(r.updatedAt)) : "—";
        return (
          '<div class="card"><h3><span class="wave">' +
          escHtml(m.wave) +
          "</span> " +
          escHtml(title) +
          '</h3><div class="meta">Registry · <code>' +
          sched +
          "</code> · updated " +
          upd +
          "</div></div>"
  ...
  function rememberOverviewActivityStatic() {
    const el = document.getElementById("overview-activity-mount");
    if (el && overviewActivityStaticHtml === null) overviewActivityStaticHtml = el.innerHTML;
  }

  function restoreOverviewActivityStaticIfStored() {
    const el = document.getElementById("overview-activity-mount");
    if (el && overviewActivityStaticHtml !== null) el.innerHTML = overviewActivityStaticHtml;
  }

  function formatOverviewActivityWhen(iso) {
    try {
  ...
  /** Apply <code>GET /api/v1/overview/summary</code> — KPI subtitles, license tile, activity, DD device pulse. */
  function applyOverviewSummary(summary) {
    if (!summary || typeof summary !== "object") return;
    const inc = summary.incidents || {};
    const opened = inc.openedLast7d != null ? inc.openedLast7d : 0;
    const resolved = inc.resolvedLast7d != null ? inc.resolvedLast7d : 0;
    setElText("mockup-live-kpi-overview-incidents-sub", "Live · " + opened + " opened / " + resolved + " resolved (7d)");
    const p12 = inc.p1P2Open != null ? inc.p1P2Open : 0;
    setElText(
      "mockup-live-kpi-overview-p12-sub",
      p12 >= 1 ? "Action required" : "Within target (no P1/P2 open)"
    );
    const tac = summary.tacCases || {};
    const withTac = inc.withTacNumber != null ? inc.withTacNumber : 0;
    const tacOpen = tac.open != null ? tac.open : 0;
    setElText("mockup-live-kpi-overview-tac-sub", withTac + " incidents with TAC id · " + tacOpen + " open cases");
    const dev = summary.devices || {};
    setElText(
      "mockup-live-kpi-overview-devices-sub",
      (dev.active != null ? dev.active : 0) +
        " active · " +
        (dev.maintenance != null ? dev.maintenance : 0) +
        " maintenance"
    );
    const lic = summary.licenses;
    if (lic && lic.rowCount > 0 && lic.compliancePercent != null) {
      setElText("mockup-live-kpi-overview-license", String(lic.compliancePercent) + "%");
      setElText("mockup-live-kpi-overview-license-sub", "Smart Licensing · " + lic.rowCount + " SKU row(s)");
    }
    const totalDev = dev.total != null ? dev.total : 0;
    setElText("overview-dd-devices", totalDev.toLocaleString(undefined));

    const mount = document.getElementById("overview-activity-mount");
    if (!mount) return;
    const rows = summary.recentActivity;
    if (!rows || !rows.length) {
      mount.innerHTML =
        '<div class="tl-item"><div class="tl-time">Timeline</div><div style="color:var(--muted)">No incident updates in the database yet.</div></div>';
      return;
    }
    mount.innerHTML = rows
      .map(function (r) {
        const t = formatOverviewActivityWhen(r.createdAt);
        const num = r.incidentNumber ? escHtml(String(r.incidentNumber)) : "—";
        return (
          '<div class="tl-item"><div class="tl-time">' +
          escHtml(t) +
          " · " +
          escHtml(r.actor || "User") +
          "</div><div><strong>" +
          num +
          "</strong> — " +
          escHtml(r.body || "") +
  ...
  /** Snapshot of mock Overview activity HTML before live refresh replaces it. */
  var overviewActivityStaticHtml = null;
  ...
  function applyLiveOverviewKpis(properties, devices, incidents, tacCases) {
    ...
    setElText("mockup-live-kpi-overview-devices", devices.length.toLocaleString(undefined));
  ...
  function refreshMockupLiveData() {
    ...
    rememberOverviewActivityStatic();
    ...
    Promise.all([
      ...
      mockupFetchJson("/overview/summary")
    ])
      .then(function (results) {
        var pRes = results[0];
        if (results.some(function (r) { return r.networkError; })) {
          restoreOverviewActivityStaticIfStored();
          setAppStatusMessage(
        ...
        var sumRes = results[4];

        applyLiveOverviewKpis(properties, devices, incidents, tacCases);
        if (sumRes.ok && sumRes.body && typeof sumRes.body === "object" && !Array.isArray(sumRes.body)) {
          applyOverviewSummary(sumRes.body);
        } else {
          restoreOverviewActivityStaticIfStored();
        }
        ...
        mockupFetchJson("/admin/sources").then(function (srcRes) {
          ...
          if (srcRes.ok && Array.isArray(srcRes.body)) {
            renderOverviewSyncCardsFromRegistry(srcRes.body);
          } else {
  ...
  var BUBBLE_ORDER_KEY = "helix-mockup-bubble-order";
  ...
  var bubbleDragItem = null;

  var BUBBLE_CONTAINER_DEFS = [
    { container: ".kpi-grid", item: ":scope > .kpi" },
    { container: ".qa-grid", item: ":scope > .qa-cluster" },
    { container: ".cx-next-grid", item: ":scope > .cx-next-card" },
    { container: ".role-grid", item: ":scope > .role-card" },
    { container: ".grid-cards", item: ":scope > .property-card" },
    { container: ".roi-grid", item: ":scope > .roi-card" },
    { container: "#ai-insights-mount", item: ":scope > .ai-insight-block" }
  ];

  function loadBubbleOrderStore() {
    try {
      var raw = localStorage.getItem(BUBBLE_ORDER_KEY);
      if (!raw) return {};
  ...
  function saveBubbleOrderStore(store) {
    try {
      localStorage.setItem(BUBBLE_ORDER_KEY, JSON.stringify(store));
    } catch (e) {}
  }

  function persistBubbleContainerOrder(container) {
    if (!container || !container.dataset.bubbleGroup) return;
    var store = loadBubbleOrderStore();
    store[container.dataset.bubbleGroup] = Array.from(container.children)
      .map(function (c) {
        return c.dataset.bubbleSlot;
      })
      .filter(Boolean);
    saveBubbleOrderStore(store);
  }

  function ensureBubbleSlotsForContainer(container, itemSelector, groupBase) {
    if (!container) return;
    var children = Array.from(container.querySelectorAll(itemSelector));
    if (!children.length) return;

    if (!container.dataset.bubbleGroup) {
      var ix = container.dataset.bubbleGroupIndex || "0";
      container.dataset.bubbleGroup = groupBase + "::" + ix;
    }

    var max = 0;
    children.forEach(function (el) {
      var slot = el.dataset.bubbleSlot;
      var m = slot && /-b(\d+)$/.exec(slot);
      if (m) max = Math.max(max, parseInt(m[1], 10));
    });
    children.forEach(function (el) {
      if (el.dataset.bubbleSlot) return;
      max += 1;
      el.dataset.bubbleSlot = container.dataset.bubbleGroup + "-b" + max;
    });
  }

  function applyBubbleContainerOrder(container) {
    if (!container || !container.dataset.bubbleGroup) return;
    var store = loadBubbleOrderStore();
    var order = store[container.dataset.bubbleGroup];
    if (!order || !order.length) return;

    var map = Object.create(null);
    Array.from(container.children).forEach(function (el) {
      if (el.dataset.bubbleSlot) map[el.dataset.bubbleSlot] = el;
    });
    var domIds = Array.from(container.children)
      .map(function (c) {
        return c.dataset.bubbleSlot;
      })
      .filter(Boolean);
  ...
  function hydrateBubbleContainersForView(view) {
    if (!view || !view.id) return;
    BUBBLE_CONTAINER_DEFS.forEach(function (def) {
      var containers = Array.from(view.querySelectorAll(def.container));
      containers.forEach(function (container, idx) {
        container.dataset.bubbleGroupIndex = String(idx);
        ensureBubbleSlotsForContainer(container, def.item, view.id + "::" + def.container);
        applyBubbleContainerOrder(container);
      });
    });
  }

  function hydrateInlineAiBubbleContainer() {
    var aiContainer = document.getElementById("ai-insights-mount");
    if (!aiContainer) return;
    aiContainer.dataset.bubbleGroupIndex = "0";
    ensureBubbleSlotsForContainer(aiContainer, ":scope > .ai-insight-block", "inline-ai::insights");
    applyBubbleContainerOrder(aiContainer);
  }

  function refreshBubbleCardsDraggable() {
    document.querySelectorAll("[data-bubble-slot]").forEach(function (el) {
      el.removeAttribute("draggable");
    });

    var v = document.querySelector("section.view.active");
    if (v) {
      hydrateBubbleContainersForView(v);
      v.querySelectorAll("[data-bubble-slot]").forEach(function (el) {
        el.setAttribute("draggable", "true");
      });
    }

    hydrateInlineAiBubbleContainer();
    var aiMount = document.getElementById("ai-insights-mount");
    if (aiMount) {
      aiMount.querySelectorAll("[data-bubble-slot]").forEach(function (el) {
        el.setAttribute("draggable", "true");
      });
    }
  }

  function initBubbleCardDragAndOrder() {
    document.addEventListener(
      "dragstart",
      function (e) {
        var item = e.target.closest("[data-bubble-slot]");
        if (!item) return;
        ...
        bubbleDragItem = item;
        bubbleDragItem.classList.add("is-pane-block-dragging");
        try {
          e.dataTransfer.setData("text/plain", item.dataset.bubbleSlot || "");
          e.dataTransfer.effectAllowed = "move";
        } catch (err) {}
      },
      true
    ...
    document.addEventListener("dragover", function (e) {
      if (!bubbleDragItem) return;
      var container = e.target.closest("[data-bubble-group]");
      if (!container || container !== bubbleDragItem.parentElement) return;
      e.preventDefault();
      e.dataTransfer.dropEffect = "move";
      ...
      var after = getBlockAfterElement(
        ...
        e.clientY,
        bubbleDragItem,
        ":scope > [data-bubble-slot]:not(.is-pane-block-dragging)"
      );
      if (after == null) container.appendChild(bubbleDragItem);
      else container.insertBefore(bubbleDragItem, after);
    });
    ...
    document.addEventListener("dragend", function () {
      if (!bubbleDragItem) return;
      var container = bubbleDragItem.parentElement;
      bubbleDragItem.classList.remove("is-pane-block-dragging");
      if (container && container.dataset.bubbleGroup) {
        persistBubbleContainerOrder(container);
      }
      bubbleDragItem = null;
    });
