...
class TacApiService {
  ...
  async getAccessToken(): Promise<string> {
    ...
    if (!this.isConfigured()) {
      logger.warn('TAC API: credentials not configured, using placeholder token');
      return 'unconfigured';
    }
    ...
    try {
      ...
      logger.error('TAC API: Failed to obtain access token:', error);
   ...
   * Open a new TAC case
   */
  async openCase(request: OpenCaseRequest): Promise<TacApiCase> {
    if (!this.isConfigured()) {
      // Return a mock response in development
      logger.warn('TAC API not configured — returning mock case');
    ...
    try {
      ...
      const { data } = await this.client.post<TacApiCase>('/cases', payload);
      logger.info(`TAC case opened: ${data.case_number}`);
      return data;
    } catch (error) {
      logger.error('Failed to open TAC case:', error);
      throw error;
    }
  }
   ...
   */
  async getCase(caseNumber: string): Promise<TacApiCase | null> {
    if (!this.isConfigured()) return null;

    try {
      const { data } = await this.client.get<TacApiCase>(`/cases/${caseNumber}`);
      return data;
    } catch (error) {
      logger.error(`Failed to fetch TAC case ${caseNumber}:`, error);
      return null;
    }
  }
   ...
   */
  async getOpenCases(): Promise<TacApiCase[]> {
    if (!this.isConfigured()) return [];

    try {
      const { data } = await this.client.get<{ cases: TacApiCase[] }>('/cases', {
        params: {
      ...
      logger.error('Failed to fetch open TAC cases:', error);
      return [];
    }
  }
   ...
   */
  async getCaseNotes(caseNumber: string): Promise<TacApiCaseNote[]> {
    if (!this.isConfigured()) return [];

    try {
      const { data } = await this.client.get<{ notes: TacApiCaseNote[] }>(
        `/cases/${caseNumber}/notes`
      );
      return data.notes || [];
    } catch (error) {
      logger.error(`Failed to fetch notes for TAC case ${caseNumber}:`, error);
      return [];
    }
  }
   ...
   */
  async addCaseNote(caseNumber: string, content: string, noteType = 'customer'): Promise<void> {
    if (!this.isConfigured()) {
      logger.warn('TAC API not configured — skipping addCaseNote');
      return;
    }
    ...
    try {
      ...
      logger.info(`Note added to TAC case ${caseNumber}`);
    } catch (error) {
      logger.error(`Failed to add note to TAC case ${caseNumber}:`, error);
      throw error;
    }
  }
   ...
   */
  async updateSeverity(caseNumber: string, severity: '1' | '2' | '3' | '4'): Promise<void> {
    if (!this.isConfigured()) return;

    try {
      await this.client.patch(`/cases/${caseNumber}`, { severity });
      logger.info(`TAC case ${caseNumber} severity updated to ${severity}`);
    } catch (error) {
      logger.error(`Failed to update severity for TAC case ${caseNumber}:`, error);
      throw error;
    }
  }
   ...
   * Close a TAC case
   */
  async closeCase(caseNumber: string, resolution: string): Promise<void> {
    if (!this.isConfigured()) return;
    ...
    try {
      ...
      logger.info(`TAC case ${caseNumber} closed`);
    } catch (error) {
      logger.error(`Failed to close TAC case ${caseNumber}:`, error);
      throw error;
    }
  }
...
export const tacApiService = new TacApiService();
