...
def empty_analysis_result(days_back: int = 7, error: Optional[str] = None) -> Dict:
    """Return a valid empty analysis structure so send_reports.py does not crash."""
    result = {
...
def write_analysis_result(result: Dict) -> None:
    with open(OUTPUT_FILE, "w") as f:
        json.dump(result, f, indent=2)
    print(f"Analysis saved to {OUTPUT_FILE}")


def dry_run_analysis(days_back: int = 7) -> Dict:
    """Mock data for workflow testing without live Webex API calls."""
    result = empty_analysis_result(days_back=days_back)
    result["data_available"] = True
    result["spaces_monitored"] = [
        {"title": "GES Digitized Delivery | Core Team (dry-run)", "message_count": 3, "type": "dd_space"},
    ]
    result["total_messages_analyzed"] = 3
    result["total_dd_messages"] = 3
    result["participants_tracked"] = 2
    result["analysis"] = {
        "decisions": ["Dry-run: pipeline test only — no live Webex data"],
        "action_items": ["Verify WEBEX_ACCESS_TOKEN before production run"],
        "risks": [],
        "updates": ["SpaceLift dry-run completed successfully"],
        "next_steps": ["Remove --dry-run for live reports"],
        "workstream_status": {"Digitized Delivery": "Dry-run mode"},
    }
    return result
...
"""
    ...
    def run_full_analysis(self, days_back: int = 7, scan_personal: bool = True) -> Dict:
        ...
        result["data_available"] = len(all_messages) > 0
...
def main():
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Skip API calls; write mock spacelift_analysis.json for pipeline testing",
    )

    args = parser.parse_args()

    if args.dry_run:
        print("Dry-run mode: skipping Webex API calls")
        result = dry_run_analysis(days_back=args.days)
        write_analysis_result(result)
        spacelift = WebexSpaceLift.__new__(WebexSpaceLift)
        spacelift.access_token = ""
        print(spacelift.format_for_report(result))
        return
    ...
    try:
        ...
        print(f"Error: {exc}")
        write_analysis_result(empty_analysis_result(days_back=args.days, error=str(exc)))
        sys.exit(1)

    if args.list_spaces:
        try:
            rooms = spacelift.get_all_rooms()
            spacelift.find_dd_spaces(rooms)
        except Exception as exc:
            print(f"Failed to list spaces: {exc}")
            sys.exit(1)
        return
    ...
    try:
        ...
        print("\n" + spacelift.format_for_report(result))
    except Exception as exc:
        print(f"SpaceLift analysis failed: {exc}")
        write_analysis_result(
            empty_analysis_result(days_back=args.days, error=str(exc))
        )
        sys.exit(1)
