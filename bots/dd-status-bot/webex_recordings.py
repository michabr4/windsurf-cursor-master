...
Pulls meeting recordings and transcripts from DD participants, extracts key
information using AI, and incorporates into daily status reports. Meetings
without a Webex-generated transcript fall back to Whisper on the recording
audio. Action items are extracted with Claude and auto-created as Asana
tasks (deduplicated across runs via a marker embedded in the task notes).
...
ACTION_ITEM_SYSTEM_PROMPT = """You extract concrete action items from Digitized Delivery meeting transcripts.
...
class WebexRecordingsAnalyzer:
    ...
    def get_recording_audio_url(self, recording_id: str) -> Optional[str]:
        ...
        response = self._api_get(f"{BASE_URL}/recordings/{recording_id}")
        if not response:
            return None
