#!/usr/bin/env python3
"""Generate 100 Helix logo pairs (batch 4; dark + light wordmark); geometry phase-shifted from batch 3."""
...
SLUGS = [
    "aether-loop", "beryl-braid", "coral-ring", "drift-mesh", "ember-arc", "frost-hub",
    "glyph-weave", "haze-orbit", "iris-pulse", "jade-spiral", "kelvin-path", "lotus-fork",
    "mist-ring", "nimbus-hub", "onyx-braid", "pearl-weft", "quartz-mesh", "reef-arc",
    "silk-orbit", "terra-loop", "umbra-pulse", "vega-spiral", "wisp-path", "xenon-fork",
    "yield-ring", "zircon-hub", "alula-mesh", "brine-arc", "cirrus-weave", "dune-orbit",
    "eider-loop", "flint-pulse", "glen-spiral", "hushed-path", "indigo-fork", "jolt-ring",
    "kraft-hub", "lumen-mesh", "mica-arc", "nacre-weft", "opal-orbit", "pylon-loop",
    "quill-pulse", "rune-spiral", "sable-path", "tusk-fork", "vellum-ring", "wharf-hub",
    "xylem-mesh", "yarrow-arc", "zen-braid", "alloy-weft", "brisk-hub", "chord-mesh",
    "datum-arc", "edged-orbit", "forge-loop", "helio-pulse", "henge-spiral", "idyll-path",
    "junto-fork", "krill-ring", "ledger-hub", "motif-mesh", "notch-arc", "overt-orbit",
    "prism-loop", "qubit-pulse", "radix-spiral", "surge-path", "tact-fork", "unify-ring",
    "volta-hub", "welt-mesh", "xeno-arc", "yearn-orbit", "zonal-loop", "arbor-mesh",
    "bloom-weft", "creek-arc", "delta-loop", "ethos-pulse", "fable-spiral", "grove-path",
    "haven-fork", "inlet-ring", "jasper-hub", "knoll-mesh", "lilac-arc", "moor-orbit",
    "nova-loop", "oaken-pulse", "pike-spiral", "quern-path", "ridge-fork", "silt-ring",
    "torus-hub", "upland-mesh", "vapor-arc", "wyrd-orbit",
]
...
def gallery_html() -> str:
    lines = [
        '  <h2 class="section-title">One hundred exploratory marks (batch 4)</h2>',
        '  <p class="pair-desc" style="margin-bottom:16px">Script-generated pairs share batch 3 styling; geometry uses a different template phase. Light wordmarks: <code>-light.svg</code> siblings.</p>',
        '  <div class="grid" aria-label="Batch 4 Helix logo variants, dark UI wordmarks">',
    ]
    for slug in SLUGS:
        t = slug_title(slug)
        lines.append("    <figure>")
        lines.append(
            f'      <img src="assets/helix-logo-variant-{slug}.svg" '
            f'alt="Helix {t} mark, dark UI wordmark" width="260" height="176" />'
        )
        lines.append(
            f"      <figcaption><strong>{t}</strong> — batch 4. Light UI: "
            f"<code>helix-logo-variant-{slug}-light.svg</code>.</figcaption>"
        )
        lines.append("    </figure>")
    lines.append("  </div>")
    return "\n".join(lines)
...
def main() -> None:
    ...
    for i, slug in enumerate(SLUGS):
        ...
        with open(os.path.join(OUT, f"helix-logo-variant-{slug}-light.svg"), "w", encoding="utf-8") as f:
            f.write(light)
    print("Wrote 200 files for 100 slugs in", OUT)
