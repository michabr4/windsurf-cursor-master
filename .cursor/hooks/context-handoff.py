...
def write_handoff(
    ...
    brief_lines = [
        "# Last Session Brief",
        f"**Date:** {now.split(' ')[0]}  **Agent:** Cursor  **Tier:** {tier}",
        ...
        "**Files changed:** see Files touched in `.cursor/handoff/STATE.md`",
...
def main() -> int:
    ...
    if event in ("afterAgentResponse", "stop", "postToolUse", "subagentStop"):
        print(json.dumps({"additional_context": enrichment_message(tier, usage_percent, threshold, refreshed=refreshed)}))
        return 0
